...
vignette: >
  %\VignetteIndexEntry{Chapter 02 - Simple Comparative Experiments}
...
\noindent The `t.test` function is giving us a lot:  the statistic, the degrees of freedom, the $p$-value, a confidence interval at 95%, and estimated means for the modified and unmodified mortars.  
...
$$
so for the mortar experiment we'd want $d=2$, then if each group has 10 observations we find the power below.
```{r}
library(pwr)
