...
vignette: >
  %\VignetteIndexEntry{Chapter 05 - Introduction to Factorial Designs}
...
\noindent With more work we can make a more colorful plot.
```{r fig.align='center', fig.width=6, fig.height=5}
af <- aggregate(
	df$BatteryLife,
	by=list(
...
\noindent Next, let's check the model assumptions with diagnostic plots.  The normal probability plot and the fitted versus residual don't look too bad.
...
\noindent These plots also do not look too terribly troubling, either.
...
Later in Section 5.6, the analysis is run as a $3 \times 2$ Latin Square design, and the result of this analysis is given in Table 5.25.  Here, we modify our fixed-effects approach by adding `Day` into the list of error terms for blocking.
```{r}
