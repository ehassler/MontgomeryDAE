...
vignette: >
  %\VignetteIndexEntry{Chapter 11 - Response Surface Methods and Designs}
...
\noindent The lack of fit is not significant, but let's add the two factor interaction in anyway using the `TWI` helper.
...
\noindent Finally, we examine `MolecularWeight`.  Lack of Fit is not significant for the first order model, so we shall use that.
```{r}
...
The yarn elongation experiment investigates how `Polyethylene`, `Polystyrene`, and `Polypropylene` affect yarn elongation in kilograms of force applied.  A $\{3,2\}$ simplex lattice design was used to investigate this mixture experiment.  The package `mixexp` [@mixexp] provides a way to construct mixture experiments with R ([https://www.jstatsoft.org/index.php/jss/article/view/v072c02/v72c02.pdf](https://www.jstatsoft.org/index.php/jss/article/view/v072c02/v72c02.pdf)), and does the package `qualityTools` [@qualityTools] ([https://cran.r-project.org/web/packages/qualityTools/vignettes/qualityTools.pdf](https://cran.r-project.org/web/packages/qualityTools/vignettes/qualityTools.pdf)).  Analysis of mixture experiments can use the regular `lm` function.
...
\noindent First, we examine the hardness model.
```{r message=FALSE}
library(mixexp)
...
\noindent Second, we examine the `Solids` model.
```{r}
...
\noindent Since the quadratic model leaves no pure error we can't well estimate lack of fit.  So we'll stop here.
