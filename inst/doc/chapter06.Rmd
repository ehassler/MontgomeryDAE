...
\noindent Table 6.2 gives the signs for calculating the effects. Here we replicate 3 times as in Figure 6.1. The `model.matrix` function shows us what is actually going into the linear model.
```{r}
...
str(Table6.10)
```
\noindent We add `NA` for the `Block` level , otherwise adding the center runs given in the example.
```{r}
Example6.7 <- rbind(
	Table6.10,
	c(0, 0, 0, 0, 73, NA),
...
\noindent The coded unit version produced a model where all of the terms were significant, whereas the natural units model only found the interaction significant.  Note that both models have the same $R^2$ values and so are equally good fits.  But note how the standard error on the coefficients changes between the two formulations of the model.
